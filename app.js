...
function renderKits() {
  ...
  lista.innerHTML = state.kits.length
    ? state.kits.map((kit) => {
        ...
        const resumo = calcularResumoKit(kit, 1);
        ...
        return `
          <div class="row row-stack">
            <div class="row-info">
              ...
              <span>Total do kit: ${formatCurrency(resumo.valorBrutoKit)}</span>
              <span>Desconto: ${Number(resumo.descontoPercentual || 0).toFixed(2)}% (${formatCurrency(resumo.descontoValorTotal)})</span>
              <span>Preço com desconto: ${formatCurrency(resumo.valorTotalKit)}</span>
...
function calcularResumoKit(kit, quantidadeKits = 1) {
  ...
  for (const item of kit.itens) {
    ...
    totalBruto += venda.subtotalBruto;
  }

  const descontoValorTotal = totalBruto * (descontoPercentual / 100);
  const valorTotalKit = totalBruto - descontoValorTotal;

  return {
    descontoPercentual,
    descontoValorTotal,
    valorBrutoKit: totalBruto,
    valorUnitarioKit: quantidadeKits > 0 ? valorTotalKit / Number(quantidadeKits) : 0,
    valorTotalKit,
    detalhes,
  };
}

function renderResumoCadastroKit() {
  const body = document.getElementById("kitResumoTabelaBody");
  const descontoPercentual = Number(document.getElementById("kitDescontoPercentual")?.value || 0);

  if (!body) return;

  if (!state.kitProdutosTemporarios.length) {
    body.innerHTML = `<tr><td colspan="4">Nenhum produto adicionado.</td></tr>`;
    document.getElementById("kitResumoTotal").textContent = formatCurrency(0);
    document.getElementById("kitResumoDescontoPercentual").textContent = `0,00%`;
    document.getElementById("kitResumoDescontoValor").textContent = formatCurrency(0);
    document.getElementById("kitResumoFinal").textContent = formatCurrency(0);
    document.getElementById("kitPrecoComDescontoExibicao").value = formatCurrency(0);
    return;
  }

  let totalBruto = 0;
  body.innerHTML = state.kitProdutosTemporarios.map((item) => {
    const produto = state.produtos.find((p) => p.id === item.produto_id);
    if (!produto) {
      return `<tr><td>Produto não encontrado</td><td>${item.quantidade}</td><td>${formatCurrency(0)}</td><td>${formatCurrency(0)}</td></tr>`;
    }

    const resumo = calcularResumoProduto(produto, Number(item.quantidade));
    const venda = calcularValorVendaProduto(produto, Number(item.quantidade), resumo);
    const totalLinha = venda.subtotalBruto;
  ...
  const descontoValor = totalBruto * (descontoPercentual / 100);
  const valorFinal = totalBruto - descontoValor;

  document.getElementById("kitResumoTotal").textContent = formatCurrency(totalBruto);
  document.getElementById("kitResumoDescontoPercentual").textContent = `${descontoPercentual.toFixed(2).replace('.', ',')}%`;
  document.getElementById("kitResumoDescontoValor").textContent = formatCurrency(descontoValor);
  document.getElementById("kitResumoFinal").textContent = formatCurrency(valorFinal);
  document.getElementById("kitPrecoComDescontoExibicao").value = formatCurrency(valorFinal);
}
          ...
          <tr class="details-row">
            <td colspan="7">
              <div class="detail-box">
                ...
                `).join("")}
                <div class="row-cost-grid" style="margin-top:12px;">
                  <div><span>Total do kit</span><strong>${formatCurrency(item.subtotalBruto + item.desconto)}</strong></div>
                  <div><span>Desconto do kit</span><strong>${formatCurrency(item.desconto)}</strong></div>
                  <div><span>Preço final</span><strong>${formatCurrency(item.valorTotal)}</strong></div>
                  <div><span>Qtd de kits</span><strong>${item.quantidade}</strong></div>
                </div>
              </div>
            </td>
          </tr>
...
window.editarKit = (id) => {
  ...
  document.getElementById("kitDescontoPercentual").value = kit.desconto_percentual || 0;
...
async function init() {
  ...
  document.getElementById("formKit").addEventListener("submit", async (e) => {
    ...
    await upsertKit({
      ...
      nome,
      desconto_percentual: Number(document.getElementById("kitDescontoPercentual").value || 0),
      itens: [...state.kitProdutosTemporarios]
    });

    e.target.reset();
    document.getElementById("kitDescontoPercentual").value = 0;
    document.getElementById("kitPrecoComDescontoExibicao").value = formatCurrency(0);
  ...
  ["kitDescontoPercentual"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener("input", renderResumoCadastroKit);
      el.addEventListener("change", renderResumoCadastroKit);
    }
  });
