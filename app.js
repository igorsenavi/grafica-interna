  ...
  if (tipo === "produto") {
    ...
    detalhesKit = (kit.itens || []).map((kitItem) => {
      ...
      if (!produto) {
        return {
          nome: "Produto",
          quantidade: Number(kitItem.quantidade) * quantidade,
          valorUnitario: 0,
          subtotal: 0,
        };
      }

      const quantidadeDentroKit = Number(kitItem.quantidade) * quantidade;
      const valorProdUnit = calcularValorUnitarioProduto(produto, quantidadeDentroKit);
      const subtotalProd = valorProdUnit * quantidadeDentroKit;

      return {
        nome: produto.nome,
        quantidade: quantidadeDentroKit,
