...
function formatPercentualVisual(valor) {
  return `${Number(valor || 0).toFixed(2)}%`;
}

function montarResumoVisualProduto(resumo, papelNome = "-") {
  return `
    <div class="cost-visual-box">
      <div class="cost-visual-title">Custos visuais</div>
      <div class="cost-visual-grid">
        <span class="cost-chip"><strong>Papel:</strong> ${papelNome}</span>
        <span class="cost-chip"><strong>Orientação:</strong> ${resumo.orientacao}</span>
        <span class="cost-chip"><strong>Capacidade/folha:</strong> ${resumo.capacidadePorFolha}</span>
        <span class="cost-chip"><strong>Folhas:</strong> ${resumo.folhasNecessarias}</span>
        <span class="cost-chip"><strong>Aproveitamento:</strong> ${formatPercentualVisual(resumo.aproveitamentoReal)}</span>
        <span class="cost-chip"><strong>Papel:</strong> ${formatCurrency(resumo.custoTotalPapel)}</span>
        <span class="cost-chip"><strong>Tinta:</strong> ${formatCurrency(resumo.custoTotalTinta)}</span>
        <span class="cost-chip"><strong>Extras:</strong> ${formatCurrency(resumo.custoTotalExtras)}</span>
        <span class="cost-chip"><strong>Mão de obra:</strong> ${formatCurrency(resumo.custoTotalMaoObra)}</span>
        <span class="cost-chip"><strong>Base:</strong> ${formatCurrency(resumo.subtotalBaseTotal)}</span>
        <span class="cost-chip"><strong>Custo total:</strong> ${formatCurrency(resumo.custoTotalPedido)}</span>
        <span class="cost-chip cost-chip-highlight"><strong>Venda sugerida:</strong> ${formatCurrency(resumo.precoSugeridoTotal)}</span>
      </div>
    </div>
  `;
}

function montarResumoVisualProdutoLinha(item) {
  if (!item?.resumoProduto) return '';
  return montarResumoVisualProduto(item.resumoProduto, item.resumoProduto?.papelNome || '-');
}

function montarResumoVisualKitLinha(item) {
  if (!item?.detalhesKit?.length) return '';
  return `
    <div class="cost-visual-box">
      <div class="cost-visual-title">Composição de custos do kit</div>
      <div class="cost-kit-list">
        ${item.detalhesKit.map((det) => `
          <div class="cost-kit-item">
            <div>
              <strong>${det.nome}</strong>
              <span>${det.quantidade} un • ${det.papelNome || '-'} • ${det.folhasNecessarias} folha(s) • aproveitamento ${formatPercentualVisual(det.aproveitamentoReal)}</span>
            </div>
            <div class="cost-kit-values">
              <span>Custo ${formatCurrency(det.custoTotal)}</span>
              <strong>Venda ${formatCurrency(det.subtotal)}</strong>
            </div>
          </div>
        `).join('')}
      </div>
...
function montarDetalhesVisuaisLinhaOrcamento(item) {
  if (item?.tipo === 'produto') return montarResumoVisualProdutoLinha(item);
  if (item?.tipo === 'kit') return montarResumoVisualKitLinha(item);
  return '';
}

function hidratarItemOrcamentoSalvo(item) {
  const baseItem = {
    tipo: item.tipo,
  ...
  try {
    if (baseItem.tipo === 'produto') {
      const produto = state.produtos.find((p) => p.id === baseItem.itemId);
      if (produto) {
        const resumo = calcularResumoProduto(produto, baseItem.quantidade);
        baseItem.resumoProduto = {
          ...resumo,
          papelNome: state.papeis.find((p) => p.id === produto.papel_id)?.nome || '-',
        };
      }
    }

    if (baseItem.tipo === 'kit') {
      const kit = state.kits.find((k) => k.id === baseItem.itemId);
      if (kit) {
        const resumoKit = calcularResumoKit(kit, baseItem.quantidade);
        baseItem.detalhesKit = resumoKit.detalhes;
      }
    }
  } catch (error) {
    console.error('Não foi possível hidratar os custos visuais do item salvo.', error);
  }
...
function renderProdutos() {
  ...
  lista.innerHTML = state.produtos.length
    ? state.produtos.map((item) => {
      const papel = state.papeis.find((p) => p.id === item.papel_id);
      let custosHtml = '<div class="cost-visual-box"><div class="cost-visual-title">Custos visuais</div><p class="empty">Não foi possível calcular os custos deste produto agora.</p></div>';

      try {
        const resumo = calcularResumoProduto(item, 1);
        custosHtml = montarResumoVisualProduto(resumo, papel?.nome || '-');
      } catch (error) {
        console.error(`Erro ao calcular custos do produto ${item.nome}:`, error);
      }
...
function calcularResumoKit(kit, quantidadeKits = 1) {
  ...
  for (const item of kit.itens) {
    ...
    detalhes.push({
      ...
      folhasNecessarias: resumo.folhasNecessarias,
      aproveitamentoReal: resumo.aproveitamentoReal,
      papelNome: state.papeis.find((p) => p.id === produto.papel_id)?.nome || '-',
    });
...
function renderTabelaOrcamento() {
  ...
  if (!state.itensOrcamento.length) {
    ...
    body.innerHTML = state.itensOrcamento.map((item, index) => {
      const detalhesVisuais = montarDetalhesVisuaisLinhaOrcamento(item);
      return `
      <tr>
        <td>${item.nome}</td>
        <td>${item.tipo}</td>
      ...
      ${detalhesVisuais ? `<tr class="cost-details-row"><td colspan="7">${detalhesVisuais}</td></tr>` : ''}
    `}).join("");
  }
...
window.abrirOrcamentoSalvo = (id) => {
  ...
  state.itensOrcamento = (orc.itens || []).map((item) => hidratarItemOrcamentoSalvo(item));
