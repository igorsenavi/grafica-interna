    ...
    ? state.produtos.map((item) => `
      <div class="row">
        <div class="row-info">
          ...
          <span>Capacidade por folha: ${item.capacidade_por_folha || "Automática"}</span>
        </div>
        <div class="row-actions">
...
function calcularTotaisProduto(produto, quantidade) {
  ...
  return {
    valorUnitario: calc.precoSugeridoTotal / Number(quantidade),
    subtotalBruto: calc.precoSugeridoTotal,
    folhasNecessarias: calc.folhasNecessarias,
    capacidadePorFolha: calc.capacidadePorFolha,
    aproveitamentoReal: calc.aproveitamentoReal,
    custosDetalhados: {
      papelNome: papel?.nome || 'Papel',
      orientacao: calc.orientacao,
      quantidade: Number(quantidade),
      folhasNecessarias: calc.folhasNecessarias,
      capacidadePorFolha: calc.capacidadePorFolha,
      aproveitamentoReal: calc.aproveitamentoReal,
      custoTotalPapel: calc.custoTotalPapel,
      custoTotalTinta: calc.custoTotalTinta,
      custoTotalImpressao: calc.custoTotalImpressao,
      custoTotalExtras: calc.custoTotalExtras,
      subtotalBaseTotal: calc.subtotalBaseTotal,
      acrescimoFixoTotal: calc.acrescimoFixoTotal,
      acrescimoVariavelTotal: calc.acrescimoVariavelTotal,
      lucroTotal: calc.lucroTotal,
      precoSugeridoTotal: calc.precoSugeridoTotal,
    },
  };
}

function calcularTotaisKit(kit, quantidadeKits) {
  let subtotalBruto = 0;
  const detalhesKit = [];
  ...
  for (const item of kit.itens || []) {
    ...
    const totaisProduto = calcularTotaisProduto(produto, quantidadeTotalProduto);
    subtotalBruto += totaisProduto.subtotalBruto;

    detalhesKit.push({
      nome: produto.nome,
      quantidade: quantidadeTotalProduto,
      valorUnitario: totaisProduto.valorUnitario,
      subtotal: totaisProduto.subtotalBruto,
    });
  }

  return {
    valorUnitario: subtotalBruto / Number(quantidadeKits),
    subtotalBruto,
    detalhesKit,
  };
}
...
function renderTabelaOrcamento() {
  ...
  if (!state.itensOrcamento.length) {
    ...
    body.innerHTML = state.itensOrcamento.map((item, index) => {
      const detalhesProduto = item.tipo === "produto" && item.custosDetalhados
        ? `
          <div style="margin-top:10px; padding:12px; border:1px solid #e2e8f0; border-radius:12px; background:#f8fafc;">
            <div style="font-weight:700; margin-bottom:8px;">Custos do produto</div>
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:8px; font-size:12px; color:#334155;">
              <div><strong>Papel:</strong> ${item.custosDetalhados.papelNome}</div>
              <div><strong>Orientação:</strong> ${item.custosDetalhados.orientacao}</div>
              <div><strong>Folhas:</strong> ${item.custosDetalhados.folhasNecessarias}</div>
              <div><strong>Cap. por folha:</strong> ${item.custosDetalhados.capacidadePorFolha}</div>
              <div><strong>Aproveitamento:</strong> ${Number(item.custosDetalhados.aproveitamentoReal || 0).toFixed(2)}%</div>
              <div><strong>Papel:</strong> ${formatCurrency(item.custosDetalhados.custoTotalPapel)}</div>
              <div><strong>Tinta:</strong> ${formatCurrency(item.custosDetalhados.custoTotalTinta)}</div>
              <div><strong>Extras:</strong> ${formatCurrency(item.custosDetalhados.custoTotalExtras)}</div>
              <div><strong>Base:</strong> ${formatCurrency(item.custosDetalhados.subtotalBaseTotal)}</div>
              <div><strong>Fixos:</strong> ${formatCurrency(item.custosDetalhados.acrescimoFixoTotal)}</div>
              <div><strong>Variáveis:</strong> ${formatCurrency(item.custosDetalhados.acrescimoVariavelTotal)}</div>
              <div><strong>Lucro:</strong> ${formatCurrency(item.custosDetalhados.lucroTotal)}</div>
            </div>
          </div>
        `
        : "";

      const detalhesKit = item.tipo === "kit" && item.detalhesKit?.length
        ? `
          <div style="margin-top:10px; padding:12px; border:1px solid #e2e8f0; border-radius:12px; background:#f8fafc;">
            <div style="font-weight:700; margin-bottom:8px;">Composição e custos do kit</div>
            ${item.detalhesKit.map((det) => `
              <div style="padding:10px 0; border-top:1px solid #e2e8f0; ${det === item.detalhesKit[0] ? 'border-top:0;' : ''}">
                <div style="display:flex; justify-content:space-between; gap:12px; font-size:13px; margin-bottom:6px;">
                  <span><strong>${det.nome}</strong> • ${det.quantidade} un</span>
                  <strong>${formatCurrency(det.subtotal)}</strong>
                </div>
                ${det.custosDetalhados ? `
                  <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:8px; font-size:12px; color:#334155;">
                    <div><strong>Folhas:</strong> ${det.custosDetalhados.folhasNecessarias}</div>
                    <div><strong>Cap. por folha:</strong> ${det.custosDetalhados.capacidadePorFolha}</div>
                    <div><strong>Aproveitamento:</strong> ${Number(det.custosDetalhados.aproveitamentoReal || 0).toFixed(2)}%</div>
                    <div><strong>Papel:</strong> ${formatCurrency(det.custosDetalhados.custoTotalPapel)}</div>
                    <div><strong>Tinta:</strong> ${formatCurrency(det.custosDetalhados.custoTotalTinta)}</div>
                    <div><strong>Extras:</strong> ${formatCurrency(det.custosDetalhados.custoTotalExtras)}</div>
                    <div><strong>Fixos:</strong> ${formatCurrency(det.custosDetalhados.acrescimoFixoTotal)}</div>
                    <div><strong>Variáveis:</strong> ${formatCurrency(det.custosDetalhados.acrescimoVariavelTotal)}</div>
                    <div><strong>Lucro:</strong> ${formatCurrency(det.custosDetalhados.lucroTotal)}</div>
                  </div>
                ` : ''}
              </div>
            `).join('')}
      ...
      return `
        <tr>
          <td>${item.nome}${detalhesProduto || detalhesKit}</td>
          <td>${item.tipo}</td>
...
function renderPreviewOrcamento() {
  ...
  previewItens.innerHTML = state.itensOrcamento.map((item) => {
    ...
    const detalhesProdutoPreview = item.tipo === "produto" && item.custosDetalhados
      ? `
        <div style="margin-top:8px; padding-left:12px; font-size:12px; color:#475569;">
          <div>Folhas: ${item.custosDetalhados.folhasNecessarias} • Cap./folha: ${item.custosDetalhados.capacidadePorFolha} • Aproveitamento: ${Number(item.custosDetalhados.aproveitamentoReal || 0).toFixed(2)}%</div>
          <div>Papel ${formatCurrency(item.custosDetalhados.custoTotalPapel)} • Tinta ${formatCurrency(item.custosDetalhados.custoTotalTinta)} • Extras ${formatCurrency(item.custosDetalhados.custoTotalExtras)} • Lucro ${formatCurrency(item.custosDetalhados.lucroTotal)}</div>
        </div>
      `
      : "";
...
window.abrirOrcamentoSalvo = (id) => {
  ...
  state.itensOrcamento = (orc.itens || []).map((item) => ({
    ...
    detalhesKit: item.detalhesKit || item.detalhes_kit || [],
    custosDetalhados: item.custosDetalhados || item.custos_detalhados || null,
  }));
...
window.editarProduto = (id) => {
  ...
  document.getElementById("produtoCapacidadeFolha").value = item.capacidade_por_folha || "";
...
async function init() {
  ...
  document.getElementById("formProduto").addEventListener("submit", async (e) => {
    ...
    await upsertProduto({
      ...
      capacidade_por_folha: Number(document.getElementById("produtoCapacidadeFolha").value || 0),
    ...
    e.target.reset();
    document.getElementById("produtoCapacidadeFolha").value = "";
  ...
  document.getElementById("btnAdicionarItemOrcamento").addEventListener("click", () => {
    ...
    if (tipo === "produto") {
      ...
      nome = produto.nome;
      const totaisProduto = calcularTotaisProduto(produto, quantidade);
      valorUnitario = totaisProduto.valorUnitario;
      subtotalBruto = totaisProduto.subtotalBruto;
      var custosDetalhados = totaisProduto.custosDetalhados;
    } else {
      const kit = state.kits.find((k) => k.id === itemId);
      if (!kit) return;

      nome = kit.nome;
      const totaisKit = calcularTotaisKit(kit, quantidade);
      valorUnitario = totaisKit.valorUnitario;
      subtotalBruto = totaisKit.subtotalBruto;
      detalhesKit = totaisKit.detalhesKit;
      var custosDetalhados = null;
    }
