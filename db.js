  ...
  if (atual?.id) {
    const { data, error } = await supabase
      ...
      .update({
        ...
        dias_trabalhados_mes: payload.dias_trabalhados_mes,
        horas_trabalhadas_dia: payload.horas_trabalhadas_dia,
        custo_cnpj_mensal: payload.custo_cnpj_mensal,
      })
      .eq("id", atual.id)
      .select()
      .single();
  ...
  if (produtoId) {
    const { error } = await supabase
      ...
      .update({
        ...
        lucro: payload.lucro,
        preco_venda_manual: payload.preco_venda_manual,
        tempo_producao_minutos: payload.tempo_producao_minutos,
        capacidade_manual_folha: payload.capacidade_manual_folha,
      })
      .eq("id", produtoId);
...
export async function upsertKit(payload) {
  ...
  if (kitId) {
    const { error } = await supabase
      .from("kits")
      .update({ nome: payload.nome, desconto_percentual: payload.desconto_percentual ?? 0 })
      .eq("id", kitId);
    ...
    const { data, error } = await supabase
      .from("kits")
      .insert([{ user_id: userId, nome: payload.nome, desconto_percentual: payload.desconto_percentual ?? 0 }])
      .select()
      .single();
